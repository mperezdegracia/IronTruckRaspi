...
MbPage {
	...
	model: VisualItemModel {
		...
		MbSpinBox {
			description: qsTr("Test run interval")
		...
		MbItemOptions {
			description: qsTr("Skip test run if has been running for")
		...
		MbEditBoxDateTime {
			description: qsTr("Test interval start date")
			format: "yyyy-MM-dd"
			bind: Utils.path(root.bindPrefix, "/TestRun/StartDate")
		}

		MbEditBoxTime {
			description: qsTr("Start time")
			bind: Utils.path(root.bindPrefix, "/TestRun/StartTime")
		}
		MbEditBoxTime {
			id: testDuration
			description: qsTr("Test duration (hh:mm)")
			bind: Utils.path(bindPrefix, "/TestRun/Duration")
