...
MbPage {
	...
	model: VisualItemModel {
		...
		// Name is not available from "/AutoSelectedBatteryMeasurement"
		// As it is the same, get the autoselected battery monitor name from
		// "/AutoSelectedBatteryService".
		// TODO: Get auto selected service name from *BatteryMeasurement*.
		MbItemText {
			text: qsTr("Auto selected: %1").arg(autoSelectedMonitorName)
			wrapMode: Text.WordWrap
			horizontalAlignment: Text.AlignLeft
			show: monitorService.value === "default"
		}
		...
		MbSubMenu {
			description: qsTr("Periodic test run")
			item {
