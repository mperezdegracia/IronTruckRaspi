...
MbPage {
	...
	model: VisualItemModel {
		...
		MbEditBoxTime {
			...
			bind: Utils.path(settingsBindPrefix, "/QuietHours/StartTime")
		...
		MbEditBoxTime {
			...
			bind: Utils.path(settingsBindPrefix, "/QuietHours/EndTime")
			show: timeZones.checked
			writeAccessLevel: User.AccessUser
		}
		...
		MbEditBox {
			...
			text: Math.round(totalRuntime.value / 60 / 60)
			matchString: "0123456789"
			maximumLength: 6
			ignoreChars: "h"
			enabled: state.value === 0
			onTextChanged: totalRuntime.setValue(parseInt(text, 10)  * 60 * 60)
